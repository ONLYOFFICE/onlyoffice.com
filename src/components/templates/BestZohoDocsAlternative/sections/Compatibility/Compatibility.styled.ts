...
const StyledCompatibilityCompanySwitcherItem = styled.button<{
  ...
  &::before {
    ...
    display: inline-block;
    vertical-align: middle;
    width: ${({ $type }) => ($type === "onlyOffice" ? "28px" : "64px")};
    height: 28px;
    margin-right: 10px;
    background-repeat: no-repeat;
    background-position: center;
...
const StyledCompatibilityExample = styled.div<{
  $isOO: boolean;
  $isDocx: boolean;
  $type: "onlyOfficeDocx" | "onlyOfficeXlsx" | "zohoDocsDocx" | "zohoDocsXlsx";
}>`
  position: absolute;
  width: 100%;
  height: 100%;
  ...
  ${({ $isOO, $isDocx, $type }) => {
    const isVisible = {
      ...
      zohoDocsDocx: !$isOO && $isDocx,
      zohoDocsXlsx: !$isOO && !$isDocx,
    }[$type];
