...
interface IDesktopUninstalledData {
  experienced_technical_problems: "0" | "1";
  i_will_use_another_desktop_software: "0" | "1";
  did_not_find_necessary_features: "0" | "1";
  privacy_concerns_or_legal_violation: "0" | "1";
  i_rarely_use_onlyoffice_desktop_editors: "0" | "1";
  other: "0" | "1";
  message: string;
  email: string;
...
export default async function handler(
  ...
  try {
    ...
    if (req.headers.referer) {
      try {
        const desktopUninstalledData: IDesktopUninstalledData = {
        ...
        await db.teamlabsite.query(
          "INSERT INTO desktop_uninstalled_request SET ?",
          [desktopUninstalledData],
        );
      } catch (error) {
