import { serverSideTranslations } from "next-i18next/serverSideTranslations";
import { useTranslation } from "next-i18next";
import { ILocale } from "@src/types/locale";
import { Layout } from "@src/components/Layout";
import { AdventAnnounce } from "@src/components/modules/AdventAnnounce";
import { Head } from "@src/components/modules/head/Head";
import { Header } from "@src/components/modules/Header";
import {
  DownloadCommercialNewTemplate,
  IDownloadCommercialNewTemplate,
} from "@src/components/templates/DownloadCommercialNew";
import { Footer } from "@src/components/modules/Footer";
import { getProductsDetails } from "@src/lib/requests/getProductsDetails";
import {
  docsEnterpriseProducts,
  docsDeveloperProducts,
} from "@src/components/templates/DownloadCommercialNew/sections/Hero/data/docsData";
import { docspaceProducts } from "@src/components/templates/DownloadCommercialNew/sections/Hero/data/docspaceData";
import { IDownloadBlock } from "@src/components/widgets/download/DownloadBlock";

const DownloadCommercialNewPage = ({
  locale,
  docsEnterpriseProducts,
  docsDeveloperProducts,
  docspaceProducts,
}: ILocale & IDownloadCommercialNewTemplate) => {
  const { t } = useTranslation("download-commercial-new");

  return (
    <Layout>
      <Layout.AdventAnnounce>
        <AdventAnnounce locale={locale} />
      </Layout.AdventAnnounce>
      <Layout.Head>
        <Head title={t("PageTitle")} description={t("PageDescription")} />
      </Layout.Head>
      <Layout.Header>
        <Header locale={locale} />
      </Layout.Header>
      <Layout.Main>
        <DownloadCommercialNewTemplate
          docsEnterpriseProducts={docsEnterpriseProducts}
          docsDeveloperProducts={docsDeveloperProducts}
          docspaceProducts={docspaceProducts}
        />
      </Layout.Main>
      <Layout.Footer>
        <Footer locale={locale} />
      </Layout.Footer>
    </Layout>
  );
};

export async function getStaticProps({ locale }: ILocale) {
  const productsDetails = await getProductsDetails({
    key: [
      "onlyoffice4enterprise/documentserver-ee",
      "DocumentServer",
      "DocSpace",
    ],
  });

  const mapProducts = (products: IDownloadBlock[]) =>
    products.map((item) => {
      const nameKey = Array.isArray(item.nameKey)
        ? item.nameKey[0]
        : item.nameKey;
      const db = nameKey ? productsDetails?.[nameKey] : undefined;
      return {
        ...item,
        version: db?.version?.trim() || item.version,
        releaseDate: db?.lastmodified || item.releaseDate,
      };
    });

  return {
    props: {
      ...(await serverSideTranslations(locale, [
        "common",
        "download-commercial-new",
        "DownloadCard",
      ])),
      locale,
      docsEnterpriseProducts: mapProducts(docsEnterpriseProducts),
      docsDeveloperProducts: mapProducts(docsDeveloperProducts),
      docspaceProducts: mapProducts(docspaceProducts),
    },
  };
}

export default DownloadCommercialNewPage;
